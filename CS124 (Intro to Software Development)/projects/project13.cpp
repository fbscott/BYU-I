...
*
*    I was never able to get this to work. Couldn't figure it out.
************************************************************************/
...
void editCell(char grid[][9],
              char coords[]);
void possibleValues(char grid[][9],
                    char coords[],
                    int xCoord,
                    int yCoord,
                    int cellValue);
...
void editCell(char grid[][9], char coords[])
{
   ...
   // find possibl values
   possibleValues(grid, coords, xCoord, yCoord, cellValue);
 ...
 * POSSIBLE VALUES
 */
void possibleValues(char grid[][9], char coords[], int xCoord, int yCoord, int cellValue)
{
   // row, column, area arrays
   char r[9];
   char c[9];
   char a[9];
   ...
   for (int i = 0; i < 9; i++)
   {
      // row, column, and area arrays
      r[i] = grid[yCoord][i];
      c[i] = grid[i][xCoord];
      a[i]; // need to figure out how to populate this
   }

   for (int i = 0; i < 9; i++)
   {
      if (r[i] == cellValue || c[i] == cellValue)
      {
         cout << "ERROR: Value '"
              << cellValue
              << "' in square '"
